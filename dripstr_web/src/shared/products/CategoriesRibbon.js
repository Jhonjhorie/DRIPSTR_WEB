...
const CategoriesRibbon = ({ active }) => {
  const itemsToShow = useResponsiveItems({ mb:5, sm: 6, md: 4, lg: 9 }); 
  const itemsPerScreen = itemsToShow; 
  ...
  return (
    <div className="relative w-[95%] md:w-[90%] justify-center items-center bg-slate-50 rounded-full shadow-lg px-2 md:px-3 gap-2 md:gap-8 flex">
      {/* Title */}
        ...
        <div className="flex flex-wrap justify-start w-full space-x-1 md:space-x-4 overflow-hidden">
          {getVisibleItems().map((category, index) => (
            <Link
              to="/"
              key={index}
              className={`${active == category.label ? 'opacity-100 border-1 border-primary-color hover:border-[1px] rounded-none hover:rounded-lg' : 'opacity-70 border-secondary-color hover:border-[1px] rounded-none hover:rounded-lg'} hover:opacity-100 md:w-24 md:h-28 sm:w-16 sm:h-20 w-12 h-16 flex flex-col items-center justify-center bg-slate-50   shadow-sm  hover:shadow-lg hover:border-[1px] duration-300 transition-all ease-in-out`}
            >
              <img
                src={category.icon}
                alt={category.label}
              ...
              />
              <p className={`${active == category.label ? 'text-primary-color':'text-gray-700'} text-sm font-medium`}>{category.label}</p>
            </Link>
          ))}
        </div>

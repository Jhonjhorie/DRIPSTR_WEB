...
function Payout() {
    ...
    const [artistCashout, setArtistCashout] = useState([]);
    ...
    const fetchArtistCashout = async () => {
        const { data, error } = await supabase
            .from('artist_Cashout')
            .select('id, created_at, full_Name, owner_Id(mobile), qty, reason, status, wallet_Id(id, revenue)')
            .eq('status', 'Pending');
        if (error) {
            console.error("Error fetching cashout:", error);
        } else {
            setArtistCashout(data);
        }
    };

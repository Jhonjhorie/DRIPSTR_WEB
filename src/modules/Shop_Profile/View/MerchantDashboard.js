      ...
      try {
        ...
        if (products.length > 0) {
          const { data: orders, error: orderError } = await supabase
            .from("orders")
            .select(
              "id, prod_num, date_of_order, shipping_status, quantity, total_price"
            )
            .in(
              "prod_num",
          ...
          if (orderError) {
            ...
            const validOrders = orders.filter(
              (order) => order.shipping_status !== "Cancel"
            );

            totalOrders = validOrders.length;
            totalOrderQuantity = validOrders.reduce(
      ...
      <div className="h-auto w-full md:flex gap-2  place-items-center p-2 align-middle ">
        <div className=" h-full w-full p-2  md:pl-10">
          <div className="flex  items-center gap-2">
            <div className="text-3xl md:text-5xl font-semibold text-custom-purple mb-5">
              Dashboard
            </div>
            <div>
              <Subs />

...
const CharacterCustomization = () => {
  ...
  const bodyTypeOptions = useMemo(() => ({
    Boy: {
      ...
      Broad: "/3d/avatars/guyz/Heavy.glb",
  ...
  useEffect(() => {
    const defaultHairKey = Object.keys(hairOptions)[0];  
  ...
  const handleSave = async () => {
    try {
      ...
      const characterData = {
        ...
        hair: selectedHair, // Save the hair name (key)
        skinColor,
        hairColor,
      ...
      const { data, error } = await supabase
        .from("avatars") // Replace with your actual table name
      ...
      <div className="p-4 flex flex-1 flex-row min-h-screen bg-slate-200">
        <div className="flex-1 h-[500px] rounded-lg shadow-lg bg-gray-100">
          ...
          >
            ...
            <hemisphereLight intensity={1} skyColor={0xffffff} groundColor={0x888888} />
            <directionalLight intensity={1.2} position={[0, 0, 1]} />
             <group>
              {selectedHair && hairOptions[selectedHair] && (
                <Part
                  url={hairOptions[selectedHair]}  
                  position={[0, 0.85, 0]}
                  color={hairColor}
                />
              )}

              <Part url={currentBody} position={[0, 0, 0]} color={skinColor} scale={[0.008, 0.008, 0.008]} />
            </group>
        ...
        />
          ...
          >
            {Object.entries(hairOptions).map(([key, url]) => (
          ...
          <div className="flex flex-col mt-4">
            ...
            <div className="flex space-x-2">
              {[
                ...
                <button
                  key={option.color}
                  className={`w-10 h-10 border-2 ${skinColor === option.color ? "border-blue-500" : "border-gray-300"}`}

...
const Carousel = ({ images }) => {
  ...
  return (
    <div className="w-[100%] h-80 md:w-[65%] relative rounded-md bg-slate-200 drop-shadow-lg group overflow-hidden">
        ...
        {images.map((item, index) => (
          <div key={index} className="flex-shrink-0 h-full w-full ">
            <img
              ...
              className="w-[80%] h-[80%] md:w-[90%] md:h-[90%] lg:w-full lg:h-full object-contain z-20 group-hover:scale-90 duration-300 transition-all relative left-10 lg:left-0 lg:top-0 md:left-5 md:top-4 top-4 group-hover:left-24"
            />
            {item.product && (
             <p className={`relative text-secondary-color text-4xl bottom-3 left-4 md:text-6xl lg:text-8xl lg:bottom-28 md:bottom-10 sm:left-2 font-bold group-hover:bottom-3 lg:group-hover:bottom-16 md:group-hover:bottom-8 group-hover:text-[2.5rem] md:group-hover:text-5xl line-clamp-1 group-hover:text-primary-color  duration-300 transition-all`}>
                {item.product}
              </p>
            )}
            {item.rate && (
              <div className='bottom-52 md:bottom-[22rem] lg:bottom-96 right-0 lg:right-4 flex flex-row relative justify-end space-x-0 md:space-x-[-1rem] group-hover:bottom-[14rem] md:group-hover:bottom-[22rem] group-hover:right-0 duration-300 transition-all items-start'>
      ...
      <Logo
            className='-z-10 absolute left-8 top-12 lg:top-10 opacity-100 drop-shadow-none group-hover:drop-shadow-lg duration-300 transition-all group-hover:scale-110 w-[50%] h-[50%] object-contain'
           />
        <img src={require('@/assets/images/others/carouselframe.png')} 
            ...
            className='-z-20 absolute right-24 md:right-16 lg:right-96 top-16 lg:top-10 opacity-50 drop-shadow-none group-hover:drop-shadow-lg duration-300 transition-all group-hover:scale-110 w-[15%] h-[50%] object-contain'/>

...
const Carousel = () => {
  ...
  return (
    <div className="w-[100%] h-80 md:w-[65%] relative rounded-md bg-slate-200 drop-shadow-lg group overflow-hidden">
      ...
      {/* Left Arrow */}
      <button onClick={goToPrevious} className="absolute top-1/2 left-4 transform -translate-y-1/2 bg-gray-800 bg-opacity-50 text-white rounded-full p-2 hover:bg-opacity-70">
        <FontAwesomeIcon icon={faChevronLeft} size={28} color="#000" />
      </button>

      {/* Right Arrow */}
      <button onClick={goToNext} className="absolute top-1/2 right-4 transform -translate-y-1/2 bg-gray-800 bg-opacity-50 text-white rounded-full p-2 hover:bg-opacity-70">
        <FontAwesomeIcon icon={faChevronRight} size={28} color="#000" />
      </button>

      {/* Dots */}
      <div className="absolute bottom-0 bg-slate-300 rounded-t-lg left-[10%] flex justify-center items-center space-x-2 w-24 h-4 p-1">
        {images.map((_, index) => (
      ...
      <Logo className='-z-10 absolute left-8 top-16 lg:top-10 opacity-100 drop-shadow-none group-hover:drop-shadow-lg duration-300 transition-all group-hover:scale-110 w-[50%] h-[50%] object-contain' />
      <img src={require('@/assets/images/others/carouselframe.png')} className='-z-20 absolute left-[-2rem] bottom-[0rem] w-[100%] h-[100%] opacity-50 object-cover' />
      <img src={require('@/assets/images/others/carouselframe.png')} className='-z-20 absolute right-[-2rem] rotate-180 bottom-[0rem] w-[150%] h-[150%] opacity-10 object-cover' />

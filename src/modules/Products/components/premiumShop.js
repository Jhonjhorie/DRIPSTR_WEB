...
const PremShop = ({ shops }) => {
  ...
  const Shop = ({ shop }) => (
    <div
      ...
      className="flex flex-col items-center justify-center p-2 bg-secondary-color hover:bg-[#323232]  shadow-md border-b-2 border-gray-50 rounded-lg transition-all duration-300 cursor-pointer"
    >
      ...
      <p className="text-center text-xs md:text-sm whitespace-nowrap w-24 truncate text-slate-50 font-medium">
        {shop.shop_name}
      </p>
    </div>
  );
  ...
  return (
    <div ref={containerRef} className="w-full max-w-md mx-auto bg-[#141414] rounded-lg shadow-sm py-3 px-4 overflow-hidden">
      <div className="flex items-center w-full">
        <div><p className="font-serif tracking-widest text-sm md:text-lg text-white font-light">Dripstr's <br/> Choice</p></div>
        <div className="flex items-center ml-4 justify-between flex-grow overflow-hidden">
          ...
          <div className="flex justify-start space-x-4 overflow-hidden px-2">

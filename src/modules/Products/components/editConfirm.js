...
const EditConfirm = ({ action, item, onClose }) => {
  ...
  const handleEditBtn = async () => {
    await handleEdit(item, quantity, selectedColor, selectedSize);
    await fetchDataCart();
    onClose();
  };
  ...
  return (
    <div className="w-full  md:w-[60.40rem] h-full md:h-[27rem] bg-slate-50 rounded-lg shadow-lg z-50 overflow-hidden">
      <div className="flex flex-col md:flex-row   md:pr-0 pr-4 justify-center items-center">
        {/* Image Section */}
                ...
                <div className="flex justify-end gap-2 items-center">
                  <div className="flex gap-1">
                    <div className="flex gap-1 items-center">
                      ...
                      <RateSymbol
                        item={averageRate(item?.prod.reviews || [])}
                        size={"4"}
                      />
                    </div>
                    <h2 className="text-xs font-medium">

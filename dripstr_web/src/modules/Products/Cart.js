...
const Cart = ({ item, action }) => {
  ...
  const { cartItems, loading, error } = useCarts();
  ...
  if (isLoggedIn) {
    return (
      <div className=" bg-slate-200 text-base-content min-h-full gap-2 w-[40rem] pr-8 p-4 flex flex-col">
        ...
        <div className="h-full flex flex-col gap-2">
          {cartItems.length > 0 ? (
            cartItems.map((cartItem) => (
              <div
                key={cartItem.id}
                className="flex flex-col gap-2 pb-2 bg-primary-color w-full rounded-md"
              >
                <div className="flex flex-row  bg-slate-50 gap-2 py-3 px-2 rounded-md text-secondary-color drop-shadow-lg w-full justify-between items-center">
                  ...
                  <div className="text-center">
                    ...
                    <h1 className="text-xs font-semibold">
                      {cartItem.size.price} * {cartItem.qty}
                    </h1>
                  </div>
                </div>
              </div>

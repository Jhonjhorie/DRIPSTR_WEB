...
const CategoriesRibbon = ({ active }) => {
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-[96%] md:w-[60%] h-16 md:h-28 justify-center items-center bg-slate-50 rounded-full shadow-lg px-2 md:px-3 gap-2 md:gap-8 flex overflow-hidden"
    >
      {/* Title */}
          ...
          {Array.from({ length: totalScreens }).map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full ${
                currentScreen === index ? "bg-primary-color" : "bg-secondary-color"
              }`}
            ></button>
          ))}
        </div>

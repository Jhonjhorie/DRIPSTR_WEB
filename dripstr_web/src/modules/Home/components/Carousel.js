...
const Carousel = ({ images }) => {
  ...
  return (
    <div className="w-[100%] h-80 md:w-[65%] relative rounded-md bg-slate-200 drop-shadow-lg group overflow-hidden">
      ...
      >
        {images.map((item, index) => (
          <div key={index} className="flex-shrink-0 h-full w-full ">
            ...
            />
            {item.product && (
             <p className={`relative text-secondary-color opacity-90 text-4xl bottom-3 left-4 md:text-6xl lg:text-8xl lg:bottom-28 md:bottom-10 sm:left-2 font-bold group-hover:bottom-3 lg:group-hover:bottom-16 md:group-hover:bottom-8 group-hover:text-[2.5rem] md:group-hover:text-5xl line-clamp-1 group-hover:text-primary-color  duration-300 transition-all`}>
                {item.product}
              </p>
            )}
            {item.rate && (
            ...
            <Link
            to='/'
            className='flex flex-row w-fit relative bottom-[7.9rem] sm:bottom-[7.9rem]  md:bottom-[12rem] lg:bottom-[17.3rem] group-hover:bottom-[10.25rem] sm:group-hover:bottom-[10rem] md:group-hover:bottom-[13.5rem] lg:group-hover:bottom-[16.5rem] transition-all duration-300 left-60 sm:left-[50vw] md:left-[27vw]  lg:left-[38vw] bg-secondary-color px-6 lg:px-8  justify-end gap-2 rounded-t-md py-0.5 hover:bg-primary-color items-center'
            >
      ...
      <Logo
            className='-z-10 absolute left-8 top-16 lg:top-10 opacity-100 drop-shadow-none group-hover:drop-shadow-lg duration-300 transition-all group-hover:scale-110 w-[50%] h-[50%] object-contain'
           />
